...
const PotW = ({ ALCHEMY_API_KEY }) => {
  ...
  const [promptOfTheMoment, setPromptOfTheMoment] = useState("Your initial prompt");
  ...
  const getNfts = async () => {
    ...
    for (const { network, address } of exploreCeptor) {
      ...
      for (const nft of response.nfts) {
        await sleep(100);
        const owner = await currentAlchemy.nft.getOwnersForNft(address, nft.tokenId);
